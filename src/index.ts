    ...
    if (!parseInt(year)) {
        ...
        fetch(endpoint) // Fetches the endpoint (makes the GET request)
            ...
            .then((res: APIResponse): void => {
                ...
                console.log("\n=============================================[ Results ]=============================================");
                console.log(`» The ${chalk.yellow("closest S-T-O")} is ${chalk.yellow(`${closest.sto}°`)}, which occurs on ${chalk.yellow(`${getDay(date.getDay())} ${date.getDate()}${getDateSuffix(date.getDate())} ${getMonth(date.getMonth())} ${date.getFullYear()}`)}`); // Logs the date with the closest S-T-O to 90 degrees
                ...
                console.log(`» You can ${chalk.greenBright("view the raw data")} in ${chalk.greenBright(`${process.cwd()}\\output.txt`)} (hold ctrl and click on the file text)`);
                console.log("\n");
            })
